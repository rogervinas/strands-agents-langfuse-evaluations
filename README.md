...
The implementation is intentionally minimal — a single process, file-based session storage, no RAG (the knowledge base is short enough to inline directly in the system prompt), no MCP, no external service calls. A production agent would run behind a load balancer with multiple replicas and connect to real external services via MCP or direct API calls. The goal here is to keep the agent simple so the focus stays on observability and evaluations. The full implementation lives under [`src/banking_sentinel`](src/banking_sentinel); if you are new to Strands, follow the [Strands Agents tutorial](https://strandsagents.com/latest/documentation/docs/get-started/quick-start/) first.
